...
def validate(model, criterion, val_loader, iteration, writer):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            mel_out, mel_out_post, dec_alignments, enc_dec_alignments, gate_out = model.module.outputs(text_padded,
                                                                                                       adj_padded,
                                                                                                       mel_padded,
                                                                                                       text_lengths,
                                                                                                       mel_lengths)
    ...
    enc_alignments=dec_alignments.new_zeros(enc_dec_alignments.size(0),
                                            enc_dec_alignments.size(1),
                                            enc_dec_alignments.size(2),
                                            enc_dec_alignments.size(4),
                                            enc_dec_alignments.size(4))
